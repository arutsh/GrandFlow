...
async def list_budget_lines_endpoint_by_budget_id_client(budget_id: str, token: str) -> List[dict]:
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    r = await _client.get(
        f"{BUDGETS_SERVICE_URL}/budget-lines/by-budget/{budget_id}", headers=headers
    )
    r.raise_for_status()
    return r.json()
